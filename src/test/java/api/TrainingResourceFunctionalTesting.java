...
public class TrainingResourceFunctionalTesting {
	...
	public void before(){
		...
		date2.set(date1.get(Calendar.YEAR), date1.get(Calendar.MONTH), date1.get(Calendar.DATE),
		...
		userWrapper = new UserWrapper(u1.getUsername(), u1.getEmail(), u1.getPassword(),
				u1.getBirthDate());
	...
	public void testErrorForbiddenCreateTraining() {
		try {
			...
			new RestBuilder<Object>(RestService.URL).path(Uris.TRAINING).path(Uris.CREATE_TRAINING)
					.basicAuth(token, "").body(trainingWrapper).get().build();
			fail();
	...
	public void testErrorForbiddenDeleteTraining(){
		try {
			String token = restService.loginAdmin();
			new RestBuilder<Object>(RestService.URL).path(Uris.TRAINING).path(Uris.DELETE_TRAINING)
					.basicAuth(token, "").param("idT","1").get().build();
			fail();
		} catch (HttpClientErrorException httpError) {
			assertEquals(HttpStatus.FORBIDDEN, httpError.getStatusCode());
			System.out.println("ERROR>>>>> " + httpError.getMessage());
	...
	public void testErrorForbiddenDeleteTrainingPlayer(){
		try {
			String token = restService.loginAdmin();
			new RestBuilder<Object>(RestService.URL).path(Uris.TRAINING).path(Uris.DELETE_TRAINING_PLAYER)
					.basicAuth(token, "").param("idT","1").param("idU","1").get().build();
			fail();
		} catch (HttpClientErrorException httpError) {
			assertEquals(HttpStatus.FORBIDDEN, httpError.getStatusCode());
			System.out.println("ERROR>>>>> " + httpError.getMessage());
	...
	public void testErrorUnauthorizedShowTrainings(){
		try {
			new RestBuilder<Object>(RestService.URL).path(Uris.TRAINING).path(Uris.SHOW_TRAININGS)
					.basicAuth("", "").get().build();
			fail();
		} catch (HttpClientErrorException httpError) {
	...
	public void testErrorUnauthorizedRegisterTraining(){
		try {
			...
			new RestBuilder<Object>(RestService.URL).path(Uris.TRAINING).path(Uris.REGISTER_TRAINING)
					.basicAuth(token, "").param("idT","1").param("idU","3").get().build();
			fail();
		} catch (HttpClientErrorException httpError) {
			assertEquals(HttpStatus.UNAUTHORIZED, httpError.getStatusCode());
			System.out.println("ERROR>>>>> " + httpError.getMessage());
